...
while(curr->link!=NULL)
        ...
        temp->link=NULL;
        curr->link=temp;
        printf("Element with value %d pushed onto queue.\n",e);
        (queue->size)++;
    }
}

void queuepushfront(struct head *queue, int e)
{
    struct node* temp =(struct node*)malloc(sizeof(struct node));
    temp->data=e;
    ...
    printf("Element wit value %d pushed onto to front of the queue\n");
}
...
void queuepopback(struct head *queue)
{
    if(queue->size==-1)
    {
        printf("Queue Underflow\n");
    ...
    else if(queue->size == 0)
    {
        ...
        prev->link=NULL;
        printf("Element with value %d popped from back of queue\n",curr->data);
        queue->size=queue->size-1;
    }
